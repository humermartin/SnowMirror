...
{
    public partial class MirrorKafkaService : ServiceBase
    {
        protected static readonly ILog Log = LogManager.GetLogger(MethodBase.GetCurrentMethod()?.DeclaringType);

        Task ProcessorTask;
        bool Stopped = true;
        public MirrorKafkaService()
        {
            InitializeComponent();
        }
        ...
        public void Write(String msg, Exception e = null)
        {
            ...
            if (ConfigurationManager.AppSettings.AllKeys.Contains("LogNative"))
            {
                try
                {
                    using (var file = new System.IO.StreamWriter(@"c:\Temp\MirrorKafkaService.trace", true))
                    {
